...
<script>
            ...
            if (res.status === 422) {
             ...
             if (data.success) {
                ...
                if (presetSource === 'leave-records') {
                    ...
                    setTimeout(() => {
                        window.location.href = "/admin/employee?openModal=" + encodeURIComponent(finalName || '');
                    }, 1000);
                } else if (presetSource === 'school') {
                    setTimeout(() => {
                        window.location.href = "/admin/school?openModal=" + encodeURIComponent(finalSchoolName || '');
                    }, 1000);
                } else if (presetSource === 'position') {
                    setTimeout(() => {
                        window.location.href = "/admin/position?openModal=" + encodeURIComponent(finalPositionName || '');
                    }, 1000);
                } else if (presetSource === 'types-of-leave') {
                    setTimeout(() => {
                        window.location.href = "/admin/types-of-leave?openModal=" + encodeURIComponent(finalTypeName || '');
                    }, 1000);
                } else if (presetSource === 'remarks') {
                    setTimeout(() => {
                        window.location.href = "/admin/remarks?openModal=" + encodeURIComponent(finalRemarkName || '');
                    }, 1000);
    ...
    function initCombobox(inputId, hiddenId, dropdownId, items, opts = {}) {
        ...
        function render(filter = '') {
            ...
            highlightedIndex = -1; // Reset on search
            ...
            dropdown.querySelectorAll('.combobox-option').forEach((opt, idx) => {
                ...
                opt.addEventListener('mouseenter', () => highlightOption(idx));
            });
        }
            ...
            if ((e.key === 'Tab' || e.key === 'ArrowRight') && ghost && ghost.innerHTML) {
                ...
                if (highlightedIndex > -1 && options[highlightedIndex]) {
                    ...
                    if (val !== '') {
                        ...
                        // For multi-select, keep focus to allow adding more tags.
                        // A second Enter (on empty input) will trigger the jump.
                        if (isMulti) {
    ...
    function executeExport() {
        ...
        rows.forEach(row => {
            if (row.classList.contains('forwarded-header-row')) {
                ...
                if (hasSelected) {
                    // Add a blank spacer row before every header except the first one
                    if (!isFirstGroup) {
        ...
        const exportedRows = document.querySelectorAll('#recordsTableBody tr.record-row.print-selected');
        ...
        togglePrintMode(); // Exit excel mode (this clears print-selected classes)
