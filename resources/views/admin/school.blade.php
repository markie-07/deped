                    ...
                    <div class="hero-search-card">
                        ...
                        <div class="hero-filter">
                            <div class="filter-select-wrap">
                                ...
                                <select id="assignedFilter" onchange="fetchAndRenderSchools()">
...
<style>
    ...
    .modal-table tbody tr:hover td { background:#fafbff; color: #1e293b; }
    ...
    body.dark-mode .school-card { background: #1e293b; border-color: #334155; }
    ...
    body.dark-mode .school-card:hover { border-color: #10b981; box-shadow: 0 20px 40px -12px rgba(0,0,0,0.5); }
    ...
    body.dark-mode .modal-table tbody td { border-bottom-color: #1e293b; color: #cbd5e1; }
    body.dark-mode .modal-table tbody tr:hover td { background: #0a0f1e; }
    body.dark-mode .badge-leave { background: rgba(16, 185, 129, 0.15); color: #34d399; }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.fetchAndRenderSchools = function() {
        ...
        const apiPath = assigned === 'all' 
            ? '{{ url("/leave-records/schools") }}' 
            : `{{ url("/leave-records/schools") }}?assigned=${assigned}`;
    ...
    if (assignedFilter) {
        assignedFilter.addEventListener('change', fetchAndRenderSchools);
    }
    ...
    window.fetchSchoolRecords = function() {
        ...
        const assigned = document.getElementById('assignedFilter').value;
        const url = `{{ url("/leave-records/by-school") }}?school=${encodeURIComponent(schoolName)}&date=${encodeURIComponent(date)}&assigned=${assigned}`;
        const tbody = document.getElementById('schoolTableBody');
    ...
    window.deleteRecord = function(id) {
        ...
        .then(data => {
            if (data.success) {
                fetchSchoolRecords();
                // Refresh the main school list to update counts/visibility
