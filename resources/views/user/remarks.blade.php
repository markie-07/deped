                    ...
                    <div class="hero-search-card">
                        ...
                        <div class="hero-filter">
                            <div class="filter-select-wrap">
                                ...
                                <select id="assignedFilter" onchange="fetchRemarks()">
...
<style>
    ...
    .filter-select-wrap { position: relative; display: flex; align-items: center; background: #fff; border-radius: 14px; border: 1.5px solid #fde68a; padding: 0 14px; box-shadow: 0 4px 16px rgba(245,158,11,0.08); transition: all 0.2s; }
    .filter-select-wrap:focus-within { border-color: #f59e0b; box-shadow: 0 0 0 3px rgba(245,158,11,0.1); }
    .filter-select-wrap svg { position: absolute; left: 14px; width: 14px; height: 14px; color: #fbbf24; pointer-events: none; }
    .filter-select-wrap:focus-within svg { color: #f59e0b; }
    ...
    body.dark-mode .filter-select-wrap svg { color: #fbbf24; }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchRemarks() {
        ...
        const apiPath = assigned === 'all' ? '{{ url("/leave-records/remarks-list") }}' : `{{ url("/leave-records/remarks-list") }}?assigned=${assigned}`;
        ...
        fetch(apiPath)
            ...
            .then(data => {
                ...
                renderRemarks(searchInput.value);
            })
            .catch(err => {
    ...
    function renderRemarks(filter = '') {
        const filtered = allRemarksData.filter(r => r.remarks.toLowerCase().includes(filter.toLowerCase()));
    ...
    window.fetchRemarkRecords = function() {
        ...
        let url = `{{ url("/leave-records/by-remark") }}?remark=${encodeURIComponent(currentRemarkForModal)}&date=${encodeURIComponent(date)}`;
        if (assigned !== 'all') {
